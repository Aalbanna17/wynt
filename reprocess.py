...
async def process_specific_s3_objects():
    for key in SPECIFIC_KEYS:
        try:
            ...
            log_to_cloudwatch_logs(LOG_STREAM_NAME, f"Processing file: s3://{BUCKET_NAME}/{key}")

            file_content, metadata = get_s3_object(BUCKET_NAME, key)
            extracted_text = extract_text_from_pdf(file_content)
            logger.info(f"Extracted text from {key}: {extracted_text[:100]}...")

            await cvprocess(metadata, extracted_text)

            log_to_cloudwatch_logs(LOG_STREAM_NAME, f"Finished processing file: {key} successfully.")

        except ClientError as e:
            logger.error(f"Error processing file {key}: {str(e)}")
            log_to_cloudwatch_logs(LOG_STREAM_NAME, f"Client error occurred while processing file: {key}.")
        except Exception as e:
            logger.error(f"Unexpected error during processing file {key}: {str(e)}")
            log_to_cloudwatch_logs(LOG_STREAM_NAME, f"Unexpected error occurred while processing file: {key}.")
