...
async def store_cv_data(session, cv_data, scorejson, scoren, resumeid, tenantId, profile_type, title_categories, jobid=None, candidate_id=None):
    try:
        ...
        log_to_cloudwatch_logs(LOG_STREAM_NAME,f"Error storing CV data: {e}")
        await session.rollback()
        return False

async def handle_talent_profile(session, cv_data, resumeid, tenantId, profile_type, title_categories):
    age = parse_age(cv_data.get('Age'))
    experience = parse_experience(cv_data.get('experience'))
    ...
    if existing_talent:
        update_existing_talent(existing_talent, cv_data, resumeid, title_category_id, age, experience)
    ...
    log_to_cloudwatch_logs(LOG_STREAM_NAME,f"profile_id {profile_id} successfully.")
    return profile_id
...
async def create_new_talent(session, cv_data, resumeid, tenantId, profile_type, title_category_id, age, experience):
    ...
    log_to_cloudwatch_logs(LOG_STREAM_NAME,f"create_new_talent successfully.")
    return new_talent.id

def update_existing_talent(talent, cv_data, resumeid, title_category_id, age, experience):
    ...
    log_to_cloudwatch_logs(LOG_STREAM_NAME,f"update successfully.")
...
async def create_candidate_record(session, cv_data, resumeid, tenantId, profile_type, title_categories):
    ...
    await session.flush()
    await session.commit()
    log_to_cloudwatch_logs(LOG_STREAM_NAME,f"create candidate record profile_id{new_candidate.id}")
...
async def update_job_and_candidate(session, jobid, tenantId, candidate_id, profile_id):
    ...
    if candidate:
        ...
        log_to_cloudwatch_logs(LOG_STREAM_NAME,f"Candidate with ID {candidate_id} updated successfully.")

    else:
        print(f"No candidate found with ID {candidate_id}.")
        logger.warning(f"No candidate found with ID {candidate_id}.")
        log_to_cloudwatch_logs(LOG_STREAM_NAME,f"No candidate found with ID {candidate_id}.")


    send_eventbridge_event(profile_id, candidate_id)


def parse_age(age_str):
    return int(age_str) if isinstance(age_str, str) and age_str.isdigit() else age_str if isinstance(age_str, int) else None

def parse_experience(experience_str):
    if isinstance(experience_str, str):
        match = re.search(r'\d+', experience_str)
        return int(match.group()) if match else None
    elif isinstance(experience_str, int):
